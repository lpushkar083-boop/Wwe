...
bot = telebot.TeleBot('8971170831:AAFVMneuni4edJXS6YpUoHXLYKLoTBRPgDc')
...
def add_user(message):
    ...
    if user_id in admin_id:
        ...
        response = "ꜰʀᴇᴇ ᴋᴇ ᴅʜᴀʀᴍ ꜱʜᴀʟᴀ ʜᴀɪ ᴋʏᴀ ᴊᴏ ᴍᴜ ᴜᴛᴛʜᴀ ᴋᴀɪ ᴋʜɪ ʙʜɪ ɢᴜꜱ ʀʜᴀɪ ʜᴏ ʙᴜʏ ᴋʀᴏ ꜰʀᴇᴇ ᴍᴀɪ ᴋᴜᴄʜ ɴʜɪ ᴍɪʟᴛᴀ ʙᴜʏ:- @DDOS_KA_OWNER."
...
def start_attack_reply(message, target, port, time):
    ...
    response = f"{username}, 𝗔𝗧𝗧𝗔𝗖𝗞 𝗦𝗧𝗔𝗥𝗧𝗘𝗗.🚀\n\n𝐓𝐚𝐫𝐠𝐞𝐭: {target}\n𝐏𝐨𝐫𝐭: {port}\n𝐓𝐢𝐦𝐞: {time} 𝐒𝐞𝐜𝐨𝐧𝐝𝐬\n𝐌𝐞𝐭𝐡𝐨𝐝: VIP- @DDOS_KA_OWNER BUY VIP DDOS ✅"
    bot.reply_to(message, response)
...
def handle_attack(message):
    ...
    if user_id in allowed_user_ids:
        ...
        if len(command) == 4:  # Updated to accept target, time, and port
            ...
            if time > 60:
                response = "GROUP MEIN 60sc hai bus✅ 60."
            else:
                ...
                response = f"𝙱𝙶𝙼𝙸 𝙰𝚃𝚃𝙰𝙲𝙺 𝙻𝙰𝚄𝙽𝙲𝙷🚀🎯.Target: {target} Port: {port} Port: {time}"
        else:
            response = "✅ Usage :- /attack <target🎯> <port🔌> <time🕛>"  # Updated command syntax
    else:
        response = ("🚫 Unauthorized Access! 🚫\n\nOops! It seems like you don't have permission to use the /attack command.                                                                          DM TO BUY ACCESS✅:- @DDOS_KA_OWNER")
...
💥 /status : Please Check Before Use !!.
...
Buy From :- @DDOS_KA_OWNER
Official Channel :- https://t.me/+dfJNzg_vUBk0OTg1
'''
...
def welcome_status(message):
    user_name = message.from_user.first_name
    response = f'''{user_name} please wait status error ❌
